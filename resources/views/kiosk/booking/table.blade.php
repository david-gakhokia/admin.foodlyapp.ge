...
@section('title', 'Reserve at ' . $table->name)
...
@section('content')
    ...
    <div class="container d-flex justify-content-center align-items-center mt-" style="min-height: 80vh;">
        <div class="card p-4" style="max-width: 500px; width: 100%;">
            ...
            <h2 class="text-center mb-2 fw-bold" style="font-size: 1.2rem;">Reserv - {{ $restaurant->name }} / {{ $place->name }}/ {{ $table->name }}</h2>

            <form method="POST" action="{{ route('booking-api.table.reserve', [$restaurant->slug, $place->slug, $table->slug]) }}">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-6 mb-1">
                        ...
                        <select name="reservation_time" class="form-control @error('reservation_time') is-invalid @enderror"
                            ...
                            @php
                                $isToday = isset($reservationDate) && $reservationDate === now()->toDateString();
                                $currentTime = now()->format('H:i');
                            @endphp
                            @foreach ($availableSlots as $time)
                                <option value="{{ $time }}"
                                    {{ old('reservation_time') == $time ? 'selected' : '' }}
                                    {{ ($isToday && $time <= $currentTime) ? 'disabled style=\'color:#ccc;\'' : '' }}>
                                    {{ $time }}
                                </option>
                            @endforeach
                        </select>
