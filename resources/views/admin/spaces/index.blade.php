<x-layouts.app title="Spaces Management">
            ...
            <div class="mb-8">
                ...
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-4xl font-bold text-gray-900">Spaces</h1>
                            <p class="text-gray-600 text-lg">Manage restaurant spaces and dining areas</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $spaces->total() }}</p>
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-green-600 mt-1">{{ $spaces->where('status', 'active')->count() }}</p>
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-red-600 mt-1">{{ $spaces->where('status', 'inactive')->count() }}</p>
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-yellow-600 mt-1">{{ $spaces->where('status', 'maintenance')->count() }}</p>
                            </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
                        <div class="lg:col-span-3">
                            ...
                            <div class="relative">
                                <input
                                    ...
                                    placeholder="Type space name or slug..."
                        ...
                        <div>
                            ...
                            <select name="status" 
                                ...
                                @foreach(\App\Models\Space::getStatuses() as $value => $label)
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Space</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($spaces as $space)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <div class="h-10 w-10 rounded-lg bg-gradient-to-r from-purple-500 to-violet-600 flex items-center justify-center">
                                                    <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                                                    </svg>
                                                </div>
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $space->slug }}</div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($space->image)
                                            <img src="{{ $space->image }}" alt="Space Image" class="h-12 w-12 rounded-lg object-cover shadow-sm">
                                        @elseif ($space->image_link)
                                            <img src="{{ $space->image_link }}" alt="Space Image" class="h-12 w-12 rounded-lg object-cover shadow-sm">
                                        @else
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $space->translate($defaultLocale)?->name ?? '—' }}
                                        </div>
                                        <div class="text-xs text-gray-500 mt-1">
                                            @foreach (config('translatable.locales') as $locale)
                                                <span class="inline-block mr-2">
                                                    {{ strtoupper($locale) }}:
                                                    {{ $space->translate($locale)?->name ?? '—' }}
                                                </span>
                                            @endforeach
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 font-medium">{{ $space->rank }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <a href="{{ route('admin.spaces.restaurants.index', $space) }}"
                                               ...
                                               title="Manage Restaurants">
                                                ...
                                                Restaurants ({{ $space->restaurants_count ?? 0 }})
                                            </a>
                                            
                                            <form action="{{ route('admin.spaces.destroy', $space) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                        onclick="return confirm('Are you sure you want to delete this space? This action cannot be undone.')"
