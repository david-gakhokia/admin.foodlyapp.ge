...
<div class="p-8">
    <!-- Form Header -->
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-2">Space Information</h2>
        <p class="text-gray-600">Fill in the details below to {{ isset($space) && $space->exists ? 'update the' : 'create a new' }} space.</p>
    </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" name="{{ $locale }}[name]"
                            value="{{ old($locale . '.name', isset($space) ? $space->translate($locale)?->name : '') }}"
                            ...
                            placeholder="Enter space name in {{ strtoupper($locale) }}">
                        @error($locale . '.name')
                    ...
                    <div>
                        ...
                        <textarea name="{{ $locale }}[description]" rows="4"
                            class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500 focus:ring-opacity-20 transition-all duration-200 text-sm bg-gray-50 focus:bg-white px-4 py-3"
                            placeholder="Enter space description in {{ strtoupper($locale) }}">{{ old($locale . '.description', isset($space) ? $space->translate($locale)?->description : '') }}</textarea>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div>
                ...
                <input type="text" name="slug"
                    value="{{ old('slug', isset($space) ? $space->slug : '') }}"
                    class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500 focus:ring-opacity-20 transition-all duration-200 text-sm bg-gray-50 focus:bg-white px-4 py-3"
                    placeholder="e.g., main-dining-area, outdoor-terrace">
                @error('slug')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="status"
                    ...
                    @foreach(\App\Models\Space::getStatuses() as $value => $label)
                        <option value="{{ $value }}" @selected(old('status', isset($space) ? $space->status : 'active') === $value)>
                            @if($value === 'active') ✅ @elseif($value === 'inactive') ❌ @elseif($value === 'maintenance') 🔧 @endif
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="rank" id="rank"
                    class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500 focus:ring-opacity-20 transition-all duration-200 text-sm bg-gray-50 focus:bg-white px-4 py-3"
                    value="{{ old('rank', isset($space) ? $space->rank : '') }}" 
                    min="0" step="1" 
    ...
    <div class="border-t border-gray-200 pt-8 mb-8">
        ...
        @if (!empty($space->image) && isset($space))
            <div class="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-xl">
                <div class="flex items-center gap-3 mb-3">
                    <svg class="h-5 w-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span class="text-sm font-medium text-purple-800">Current image uploaded</span>
                </div>
                <img src="{{ $space->image }}" alt="Current Space Image" 
                     class="h-32 w-32 object-cover rounded-lg shadow-md border border-purple-200">
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">
                    {{ isset($space) && !empty($space->image) ? 'Replace Image' : 'Upload Image' }}
                </label>
            ...
            <div>
                ...
                <input type="url" name="image_link"
                    value="{{ old('image_link', isset($space) ? $space->image_link : '') }}"
                    class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500 focus:ring-opacity-20 transition-all duration-200 text-sm bg-gray-50 focus:bg-white px-4 py-3"
    ...
    <div class="border-t border-gray-200 pt-8">
        <div class="flex flex-col sm:flex-row gap-4 justify-end">
            ...
            <button type="submit"
                ...
                {{ isset($space) && $space->exists ? 'Update Space' : 'Create Space' }}
            </button>
        </div>
    </div>
</div>
        ...
        if (input.files && input.files[0]) {
            ...
            fileNameDiv.innerHTML = `
                <div class="flex items-center gap-2 p-2 bg-purple-50 border border-purple-200 rounded-lg">
                    ...
                    <span class="text-purple-600 text-xs">(${fileSize} MB)</span>
                </div>
            `;
