    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
        ...
        <div class="bg-white/90 backdrop-blur-lg rounded-3xl shadow-xl border border-white/30 p-8 mb-8">
            <div class="flex items-center mb-6">
                <div
                    class="w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mr-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400">მოძებნეთ კატეგორია სახელით და გაფილტრეთ სტატუსით</p>
                </div>
            </div>

            <form method="GET" action="{{ route('admin.restaurants.menu.categories.index', $restaurant) }}"
                class="space-y-6">
                {{-- Single Row Layout --}}
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <div class="flex-1 min-w-80">
                        <label for="search" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            <svg class="w-4 h-4 inline mr-2 text-blue-500" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                            </svg>
                    ...
                    <div class="w-48">
                        <label for="status" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            <svg class="w-4 h-4 inline mr-2 text-green-500" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button"
                            onclick="document.getElementById('search').value=''; document.getElementById('status').value=''; this.form.submit();"
                ...
                @if (request()->hasAny(['search', 'status']))
                    <div
                        ...
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="text-sm font-semibold text-blue-800 dark:text-blue-200">აქტიური
                                ფილტრები:</span>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            @if (request('search'))
                                <span
                                    class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-200">
                                    ძიება: "{{ request('search') }}"
                                </span>
                            @endif
                            @if (request('status'))
                                <span
                                    class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-full bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-200">
        ...
        {{-- Cards Grid Container - 3 cards per row with larger size --}}
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-10">
            @forelse ($menuCategories as $index => $category)
                <div
                    class="group bg-white/90 backdrop-blur-lg rounded-3xl shadow-xl border border-white/30 overflow-hidden hover:shadow-2xl transition-all duration-500 hover:scale-105 transform">
                    {{-- Category Image --}}
                    <div
                        class="relative h-64 bg-gradient-to-br from-blue-100 via-purple-50 to-indigo-100 dark:from-gray-700 dark:to-gray-600 overflow-hidden">
                        @if ($category->image)
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center relative">
                                <div class="absolute inset-0 bg-gradient-to-br from-blue-200/50 to-purple-200/50">
                                </div>
                                <div
                                    class="relative w-20 h-20 bg-white/80 backdrop-blur-sm rounded-3xl flex items-center justify-center shadow-lg">
                        ...
                        <div class="absolute top-4 right-4">
                            @if ($category->status === 'active')
                                <span
                                    class="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-full bg-green-500 text-white shadow-lg backdrop-blur-sm">
                                    <div class="w-2 h-2 bg-white rounded-full mr-2 animate-pulse"></div>
                                    აქტიური
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-full bg-gray-500 text-white shadow-lg backdrop-blur-sm">
                        ...
                        <div class="absolute top-4 left-4">
                            <span
                                class="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-full bg-white/90 text-gray-700 shadow-lg backdrop-blur-sm">
                    ...
                    <div class="p-8">
                        ...
                        <h3 class="text-2xl font-bold mb-4 line-clamp-2">
                            @if ($category->children->count() > 0)
                                <a href="{{ route('admin.restaurants.menu.categories.children', [$restaurant, $category]) }}"
                                    class="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent hover:from-blue-700 hover:to-purple-700 transition-all duration-300 group-hover:scale-105 transform inline-block">
                        ...
                        <div class="space-y-4 mb-8">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center text-base font-medium text-gray-600 dark:text-gray-400">
                                    <div
                                        class="w-10 h-10 bg-gradient-to-r from-blue-100 to-purple-100 rounded-xl flex items-center justify-center mr-4">
                                        <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center text-base font-medium text-gray-600 dark:text-gray-400">
                                    <div
                                        class="w-10 h-10 bg-gradient-to-r from-green-100 to-emerald-100 rounded-xl flex items-center justify-center mr-4">
                            ...
                            <div class="flex items-center justify-end">
                                <span
                                    class="text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-4 py-2 rounded-full font-medium">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <form
                                action="{{ route('admin.restaurants.menu.categories.destroy', [$restaurant, $category]) }}"
                                method="POST" class="col-span-2"
            ...
            @empty
                ...
                <div class="col-span-full flex flex-col items-center justify-center py-20 px-4 text-center">
                    <div
                        class="backdrop-blur-lg bg-white/20 dark:bg-gray-800/20 border border-white/30 dark:border-gray-700/30 rounded-3xl p-12 max-w-md mx-auto shadow-2xl">
                        <div
                            class="w-24 h-24 bg-gradient-to-r from-blue-100 to-purple-100 rounded-full flex items-center justify-center mx-auto mb-8 shadow-lg">
                        ...
                        <h3
                            class="text-2xl font-bold bg-gradient-to-r from-gray-700 to-gray-900 dark:from-gray-300 dark:to-gray-100 bg-clip-text text-transparent mb-4">
                            კატეგორია არ მოიძებნა
                        </h3>
                        <p class="text-gray-600 dark:text-gray-400 mb-8 text-lg leading-relaxed">
                            ამ რესტორნისთვის ჯერ არ შექმნილა მენიუს კატეგორია. დაამატეთ პირველი კატეგორია დასაწყებად.
                        </p>
                        <a href="{{ route('admin.restaurants.menu.categories.create', $restaurant) }}"
            ...
            <div class="bg-white/90 backdrop-blur-lg rounded-3xl shadow-xl border border-white/30 p-8 mt-8">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div class="text-sm text-gray-600 dark:text-gray-400">
                        გვერდი {{ $menuCategories->currentPage() }} / {{ $menuCategories->lastPage() }} 
                        (სულ {{ $menuCategories->total() }} კატეგორია)
                    </div>
