<x-layouts.app :title="'Slot-ის რედაქტირება - ' . $place->translate('ka')->name">
    <!-- Header -->
    <div class="mb-8">
        <!-- Breadcrumb -->
        <nav class="flex mb-6" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-2 bg-white/80 backdrop-blur-sm border border-gray-200/50 rounded-xl px-6 py-3 shadow-lg">
                <li class="inline-flex items-center">
                    <a href="{{ route('admin.places.index') }}" class="inline-flex items-center text-sm font-semibold text-gray-600 hover:text-blue-600 transition-all duration-200">
                        ადგილები
                    </a>
                </li>
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('admin.places.show', $place) }}" class="ml-2 text-sm font-semibold text-gray-600 hover:text-blue-600 transition-all duration-200">
                            {{ $place->translate('ka')->name ?? $place->translate('en')->name }}
                        </a>
                    </div>
                </li>
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('admin.restaurants.places.slots.index', [$place->restaurant, $place]) }}" class="ml-2 text-sm font-semibold text-gray-600 hover:text-blue-600 transition-all duration-200">
                            Slots
                        </a>
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-2 text-sm font-bold text-amber-600 bg-amber-50 px-3 py-1 rounded-xl">რედაქტირება</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <!-- Header Section -->
        <div class="bg-gradient-to-br from-white via-gray-50 to-amber-50 backdrop-blur-lg shadow-2xl border border-gray-200/50 rounded-3xl p-8 relative overflow-hidden">
            <div class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-amber-100/30 to-orange-100/30 rounded-full -translate-y-32 translate-x-32 blur-3xl"></div>
            
            <div class="relative flex items-center gap-6">
                <div class="w-20 h-20 bg-gradient-to-br from-amber-500 via-orange-600 to-red-700 rounded-3xl flex items-center justify-center shadow-2xl ring-4 ring-amber-100/50">
                ...
                <div>
                    <h1 class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-gray-800 via-gray-900 to-amber-900 mb-3 leading-tight">
                        Slot-ის რედაქტირება
                    </h1>
                    <p class="text-lg text-gray-600 font-semibold">{{ $place->translate('ka')->name ?? $place->translate('en')->name }} - 
    ...
    <!-- Form -->
    <div class="bg-gradient-to-br from-white via-gray-50 to-amber-50 backdrop-blur-lg shadow-2xl border border-gray-200/50 rounded-3xl overflow-hidden">
        <div class="bg-gradient-to-r from-amber-400 via-orange-500 to-red-600 px-8 py-6">
            <h2 class="text-2xl font-black text-white drop-shadow-md">Slot-ის ინფორმაცია</h2>
        </div>
        
        <div class="p-8">
            <form action="{{ route('admin.restaurants.places.slots.update', [$place->restaurant, $place, $slot]) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="day_of_week" id="day_of_week" 
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-100 transition-all duration-300 bg-white shadow-md @error('day_of_week') border-red-500 @enderror">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <!-- Custom time input -->
                            <div class="flex items-center gap-2 w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus-within:border-amber-500 focus-within:ring-4 focus-within:ring-amber-100 transition-all duration-300 bg-white shadow-md @error('time_from') border-red-500 @enderror">
                                <select id="time_from_hours" class="flex-1 bg-transparent border-none focus:outline-none text-lg font-mono font-bold text-gray-800">
                                    @for($i = 0; $i < 24; $i++)
                                        @php
                                            ...
                                            $selectedHour = $currentTime ? substr($currentTime, 0, 2) : null;
                                        @endphp
                                ...
                                <select id="time_from_minutes" class="flex-1 bg-transparent border-none focus:outline-none text-lg font-mono font-bold text-gray-800">
                                    @for($i = 0; $i < 60; $i += 15)
                                        @php
                                            $currentTime = old('time_from', $slot->time_from);
                                            $selectedMinute = $currentTime ? substr($currentTime, 3, 2) : null;
                                        @endphp
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <!-- Custom time input -->
                            <div class="flex items-center gap-2 w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus-within:border-amber-500 focus-within:ring-4 focus-within:ring-amber-100 transition-all duration-300 bg-white shadow-md @error('time_to') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <select name="slot_interval_minutes" id="slot_interval_minutes" 
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-100 transition-all duration-300 bg-white shadow-md @error('slot_interval_minutes') border-red-500 @enderror">
                ...
                <div class="flex gap-4 pt-6 border-t border-gray-200">
                    <button type="submit" 
                            class="flex-1 inline-flex justify-center items-center px-6 py-4 bg-gradient-to-r from-amber-500 via-orange-600 to-red-600 hover:from-amber-600 hover:via-orange-700 hover:to-red-700 text-white font-bold text-lg rounded-2xl shadow-2xl transition-all duration-300 transform hover:scale-105">
    ...
    <style>
        ...
        .custom-time-input:focus-within {
            ...
            ring: 4px;
            ring-color: rgba(245, 158, 11, 0.1);
        }
    </style>
