<x-layouts.app :title="'Dish რედაქტირება - ' . $restaurant->name">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="mb-8">
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-4xl font-bold text-gray-900">Dish რედაქტირება</h1>
                            <p class="text-gray-600 text-lg">{{ $dish->name }} - {{ $restaurant->name }}</p>
                        </div>
                    </div>
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-gray-200 shadow-lg">
                    <div class="flex items-center justify-between">
                        ...
                        <div>
                            ...
                            @else
                                <div class="w-12 h-12 bg-gradient-to-br from-green-100 to-teal-100 rounded-lg flex items-center justify-center shadow-md">
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-xl border border-gray-200 shadow-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    ...
                    <p class="text-sm text-gray-600 mt-1">შეცვალეთ restaurant-dish კავშირის status და rank</p>
                </div>
                
                <form action="{{ route('admin.restaurants.dishes.update', [$restaurant, $dish]) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-orange-50 border border-orange-200 rounded-xl p-4">
                            <h4 class="text-sm font-semibold text-orange-800 mb-2">მიმდინარე კავშირის ინფორმაცია</h4>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                ...
                                <div>
                                    <span class="text-orange-600 font-medium">Rank:</span>
                                    <span class="ml-2 text-orange-800">{{ $dish->pivot->rank ?? 0 }}</span>
                                </div>
                                <div>
                                    <span class="text-orange-600 font-medium">Added:</span>
                                    <span class="ml-2 text-orange-800">{{ $dish->pivot->created_at?->format('M d, Y') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" 
                                ...
                                <option value="active" {{ old('status', $dish->pivot->status) === 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive" {{ old('status', $dish->pivot->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                                <option value="pending" {{ old('status', $dish->pivot->status) === 'pending' ? 'selected' : '' }}>Pending</option>
                            </select>
                            @error('status')
                        ...
                        <div>
                            ...
                            <input type="number" name="rank" id="rank" min="0" step="1" 
                                   value="{{ old('rank', $dish->pivot->rank ?? 0) }}"
