<x-layouts.app :title="'ახალი Restaurant Slot - ' . ($restaurant->translate('ka')->name ?? $restaurant->translate('en')->name ?? 'რესტორანი #' . $restaurant->id)">
    <!-- Header -->
    <div class="mb-8">
        <!-- Breadcrumb -->
            ...
            <ol class="inline-flex items-center space-x-2 bg-white/80 backdrop-blur-sm border border-gray-200/50 rounded-xl px-6 py-3 shadow-lg">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-2 text-sm font-bold text-orange-600 bg-orange-50 px-3 py-1 rounded-xl">ახალი Restaurant Slot</span>
                    </div>
                </li>
            </ol>
        </nav>
            ...
            <div class="relative flex items-center gap-6">
                <div class="w-20 h-20 bg-gradient-to-br from-orange-500 via-red-600 to-pink-700 rounded-3xl flex items-center justify-center shadow-2xl ring-4 ring-orange-100/50">
                    <svg class="w-10 h-10 text-white drop-shadow-lg" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h1a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                </div>
                ...
                <div>
                    <h1 class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-gray-800 via-gray-900 to-orange-900 mb-3 leading-tight">
                        ახალი Restaurant Slot
                    </h1>
                    <p class="text-lg text-gray-600 font-semibold">{{ $restaurant->translate('ka')->name ?? $restaurant->translate('en')->name ?? 'რესტორანი #' . $restaurant->id }}-ისთვის</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-br from-white via-gray-50 to-orange-50 backdrop-blur-lg shadow-2xl border border-gray-200/50 rounded-3xl overflow-hidden">
        ...
        <div class="p-8">
            <form action="{{ route('admin.restaurants.slots.store', $restaurant) }}" method="POST" class="space-y-8">
                @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!document.getElementById('time_to_hidden').value) {
                document.getElementById('time_to_hours').value = '22';
                document.getElementById('time_to_minutes').value = '00';
                document.getElementById('time_to_hidden').value = '22:00';
            }
        });
    </script>
</x-layouts.app>